...
function removeFirstOccurrences(str, value) {
  if (str.includes(value)) {
    ...
    list.splice(str.indexOf(value), value.length);
    return list.join('');
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  ...
  const list = str.split('');
  list.splice(str.lastIndexOf(value), value.length);
  return list.join('');
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (vowels.includes(str[i])) numberOfVowels += 1;
  }
...
function isPalindrome(str) {
  const straightStr = str.replace(/\W/g, '').toLowerCase();
  const reversStr = straightStr.split('').reverse().join('');
  return straightStr === reversStr;
}
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < list.length; i += 1) {
    if (list[i].length > longestWord.length) longestWord = list[i];
  }
  return longestWord;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (char) => list[list.indexOf(char) + 13]);
}
...
function getCardId(value) {
  ...
  return cardsMap.findIndex((card) => card === value);
}
